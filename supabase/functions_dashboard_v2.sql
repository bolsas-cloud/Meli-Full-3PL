-- ============================================
-- FUNCIONES RPC PARA DASHBOARD (V2)
-- Adaptado a la estructura real de ordenes_meli
-- ============================================
-- IMPORTANTE: Ejecutar este archivo REEMPLAZA las funciones anteriores
-- ============================================
...
-- Adaptado: usa fecha_orden y calcula ventas como cantidad*precio
-- ============================================
...
BEGIN
    ...
    SELECT
        COALESCE(SUM(o.cantidad * o.precio_unitario), 0),
        COUNT(DISTINCT o.id_orden),
        COALESCE(SUM(o.cantidad), 0),
        MAX(o.fecha_orden)
    INTO v_ventas, v_ordenes, v_items, v_ultima_orden
    FROM ordenes_meli o
    WHERE DATE(o.fecha_orden) BETWEEN p_fecha_desde AND p_fecha_hasta;
...
-- Adaptado: usa fecha_orden y calcula ventas
-- ============================================
...
BEGIN
    ...
    ventas_dia AS (
        SELECT
            ...
            COALESCE(SUM(o.cantidad * o.precio_unitario), 0) AS total_ventas,
...
-- Adaptado: usa titulo y calcula ventas
-- ============================================
...
BEGIN
    ...
    SELECT
        o.id_item,
        COALESCE(o.sku, 'N/A') AS sku,
        COALESCE(o.titulo, 'Sin titulo') AS titulo,
        SUM(o.cantidad)::BIGINT AS cantidad_vendida,
        COALESCE(SUM(o.cantidad * o.precio_unitario), 0) AS total_vendido
    FROM ordenes_meli o
    WHERE DATE(o.fecha_orden) BETWEEN p_fecha_desde AND p_fecha_hasta
    GROUP BY o.id_item, COALESCE(o.sku, 'N/A'), COALESCE(o.titulo, 'Sin titulo')
...
BEGIN
    RETURN QUERY
    SELECT
        (SELECT MAX(fecha_orden) FROM ordenes_meli),
